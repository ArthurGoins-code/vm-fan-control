...
class FanController:
    def __init__(self, port, max_temp=None, min_temp=None):
        ...
        # Initialize independent control parameters
        self.independent_fan_control = config.INDEPENDENT_FAN_CONTROL
        
        if self.independent_fan_control:
            # Use separate parameters for GPU and CPU when independent control is enabled
            self.gpu_min_temp = config.GPU_MIN_TEMP
            self.gpu_max_temp = config.GPU_MAX_TEMP
            self.gpu_min_fan_speed = config.GPU_MIN_FAN_SPEED
            self.gpu_max_fan_speed = config.GPU_MAX_FAN_SPEED
            self.gpu_ramp_trigger_temp = config.GPU_RAMP_TRIGGER_TEMP
            self.gpu_ramp_trigger_fan_speed = config.GPU_RAMP_TRIGGER_FAN_SPEED
            self.gpu_curve_exponent = config.GPU_CURVE_EXPONENT
            
            self.cpu_min_temp = config.CPU_MIN_TEMP
            self.cpu_max_temp = config.CPU_MAX_TEMP
            self.cpu_min_fan_speed = config.CPU_MIN_FAN_SPEED
            self.cpu_max_fan_speed = config.CPU_MAX_FAN_SPEED
            self.cpu_ramp_trigger_temp = config.CPU_RAMP_TRIGGER_TEMP
    ...
    def get_fan_speed(self, temperature, is_gpu_temp=False):
        ...
        If independent control is enabled, uses GPU or CPU specific parameters based on is_gpu_temp flag.
        """
        # Select parameters based on independent control mode and temperature type
        if self.independent_fan_control:
            if is_gpu_temp:
                min_temp = self.gpu_min_temp
                max_temp = self.gpu_max_temp
                min_fan_speed = self.gpu_min_fan_speed
                max_fan_speed = self.gpu_max_fan_speed
                ramp_trigger_temp = self.gpu_ramp_trigger_temp
                ramp_trigger_fan_speed = self.gpu_ramp_trigger_fan_speed
                curve_exponent = self.gpu_curve_exponent
            else:
                min_temp = self.cpu_min_temp
                max_temp = self.cpu_max_temp
                min_fan_speed = self.cpu_min_fan_speed
                max_fan_speed = self.cpu_max_fan_speed
                ramp_trigger_temp = self.cpu_ramp_trigger_temp
                ramp_trigger_fan_speed = self.cpu_ramp_trigger_fan_speed
                curve_exponent = self.cpu_curve_exponent
        else:
            # Use joint control parameters (backward compatibility)
            min_temp = self.min_temp
            max_temp = self.max_temp
            min_fan_speed = self.min_fan_speed
            max_fan_speed = self.max_fan_speed
            ramp_trigger_temp = self.ramp_trigger_temp
            ramp_trigger_fan_speed = self.ramp_trigger_fan_speed
        ...
        if temperature < ramp_trigger_temp:
            # Gentle ramp from min_fan_speed up to ramp_trigger_fan_speed
            temp_range = ramp_trigger_temp - min_temp
            temp_ratio = (temperature - min_temp) / temp_range
            fan_speed = min_fan_speed + temp_ratio * (
                ramp_trigger_fan_speed - min_fan_speed
            )
        else:
            ...
            temp_range = max_temp - ramp_trigger_temp
            temp_ratio = (temperature - ramp_trigger_temp) / temp_range
            fan_speed = ramp_trigger_fan_speed + (
                temp_ratio ** (1 / curve_exponent)
            ) * (max_fan_speed - ramp_trigger_fan_speed)
